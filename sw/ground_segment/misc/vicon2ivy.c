/*
 * Copyright (C) 2017 Niek Klein Koerkamp
 * Code adapted from natnet2ivy.c
 *
...
/** \file vicon2ivy.c
*  \brief Vicon (GPS) to ivy forwarder
*
*   This receives aircraft position information through the Vicon Tracker system
* UDP stream and forwards it to the ivy bus. An aircraft with the gps
...
char *natnet_multicast_addr     = "192.168.50.5";
//uint16_t natnet_cmd_port        = 1510;
uint16_t natnet_data_port       = 51001;
uint8_t natnet_major            = 2;
uint8_t natnet_minor            = 9;

/** Logging */
FILE *fp;
char *nameOfLogfile             = "vicon_log.dat";
bool log_exists = 0;
bool must_log = 0;
...
uint32_t freq_transmit          = 100;     ///< Transmitting frequency in Hz
...
struct RigidBody {
  ...
  int itemdatasize;
  char *itemname;
  double x, y, z;                    ///< Rigid body x, y and z coordinates in meters (note y and z are swapped)
  double rotx, roty, rotz;             ///< Rigid body rotx, roty, rotz (note roty and rotz are swapped)
  float qx, qy, qz, qw;
...
void natnet_parse(unsigned char *in)
{
  ...
  // Rigid bodies
  int nRigidBodies = 0;
  memcpy(&nRigidBodies, ptr, 1); ptr += 1;
  printf_debug("nRigidBodies %d\n",nRigidBodies);
  ...
  rigidBodies[j].nMarkers = 0;
  memcpy(&rigidBodies[j].id, ptr, 1); ptr += 1;
  memcpy(&rigidBodies[j].itemdatasize, ptr, 2); ptr += 2;
  memcpy(&rigidBodies[j].itemname, ptr, 24); ptr += 24;
  memcpy(&rigidBodies[j].x, ptr, 8); ptr += 8;   //x --> Y
  memcpy(&rigidBodies[j].y, ptr, 8); ptr += 8;   //y --> Z
  memcpy(&rigidBodies[j].z, ptr, 8); ptr += 8;   //z --> X
  memcpy(&rigidBodies[j].rotx, ptr, 8); ptr += 8;  //rotx --> rotY
  ...
  printf_natnet("ori: [%3.2f,%3.2f,%3.2f]\n", rigidBodies[j].rotx, rigidBodies[j].roty, rigidBodies[j].rotz);
  ...
  if (old_rigid.x != rigidBodies[j].x || old_rigid.y != rigidBodies[j].y || old_rigid.z != rigidBodies[j].z
      || old_rigid.rotx != rigidBodies[j].rotx || old_rigid.roty != rigidBodies[j].roty || old_rigid.rotz != rigidBodies[j].rotz) {
...
//     }
     ...
     // Latency
     natnet_latency = 0.00001f; // memcpy(&natnet_latency, ptr, 4); ptr += 4;
...
//     // bool bTrackedModelsChanged = params & 0x02;         // 0x02 Actively tracked model list has changed
     ...
     printf_debug("End Packet\n");
     printf_debug("RigidBodies[0] %d: xyz %f %f %f, rotxyz %f %f %f datasize: %d\n",rigidBodies[0].id,rigidBodies[0].x,rigidBodies[0].y,rigidBodies[0].z,rigidBodies[0].rotx,rigidBodies[0].roty,rigidBodies[0].rotz,rigidBodies[0].itemdatasize);
//  }
}
...
gboolean timeout_transmit_callback(gpointer data)
{
  ...
  for (i = 0; i < MAX_RIGIDBODIES; i++) {
    ...
    if (rigidBodies[i].id >= MAX_RIGIDBODIES) {
      fprintf(stderr,
              "Could not parse rigid body %d from Vicon, because ID is higher then or equal to %d (MAX_RIGIDBODIES-1).\r\n",
    ...
    orient_eulers.phi = rigidBodies[i].rotx;
    orient_eulers.theta = rigidBodies[i].roty;
    orient_eulers.psi = rigidBodies[i].rotz;

    double_quat_of_eulers(&orient, &orient_eulers);

    rigidBodies[i].qw = orient.qi;
    rigidBodies[i].qx = orient.qx;
    rigidBodies[i].qy = orient.qy;
    rigidBodies[i].qz = orient.qz;
    ...
    double heading = -orient_eulers.psi + 0 / 57.6 -
...
static gboolean sample_data(GIOChannel *chan, GIOCondition cond, gpointer data)
{
  ...
  printf_debug("Sample Vicon UDP Data\n");
  ...
  if (bytes_data >= 2 && bytes_data >= buffer_data[1]) {
    printf_debug("Parse Vicon UDP Data\n");
...
void print_help(char *filename)
{
  static const char *usage =
    ...
    "   -multicast_addr <ip>      Vicon server multicast address (239.255.42.99)\n"
    "   -server <ip>              Vicon server IP address (255.255.255.255)\n"
    "   -version <id>             Vicon server version (2.5)\n"
    "   -data_port <port>         Vicon server data socket UDP port (1510)\n"
...
int main(int argc, char **argv)
{
  ...
  struct LlaCoor_d tracking_lla;
  tracking_lla.lat = RadOfDeg(36.005417);
  tracking_lla.lon = RadOfDeg(-78.940984);
  tracking_lla.alt = 619;
  tracking_offset_angle = 0 / 57.6;
  ltp_def_from_lla_d(&tracking_ltp, &tracking_lla);
  ...
  printf_debug("Starting Vicon listening (multicast address: %s)\n",
               natnet_multicast_addr);
